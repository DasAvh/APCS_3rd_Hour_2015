...
public class Board {
	...
	public void render(Graphics g) {
		...
		int xStart = (int) Math.max(0, game.getCamera().getxOffset()
				/ Tile.TILE_WIDTH);
		int xEnd = (int) Math.min(width,
				(game.getCamera().getxOffset() + game.getWidth())
						/ Tile.TILE_WIDTH + 1);

		// Gets starting and ending y positions of viewport
		int yStart = (int) Math.max(0, game.getCamera().getyOffset()
				/ Tile.TILE_HEIGHT);
		int yEnd = (int) Math.min(height,
				(game.getCamera().getyOffset() + game.getHeight())
						/ Tile.TILE_HEIGHT + 1);
		...
		for (int y = yStart; y < yEnd; y++) {
			for (int x = xStart; x < xEnd; x++)
				getTile(x, y).render(
						g,
						(int) (x * Tile.TILE_WIDTH - game.getCamera()
								.getxOffset()),
						(int) (y * Tile.TILE_HEIGHT - game.getCamera()
								.getyOffset()));
			// End inner for
		}// End outer for
	}// End render method
	...
	public static Tile getDoor(Entity entity) {
		return Tile.tiles[tiles[entity.getX() / Tile.TILE_WIDTH][entity.getY()
				/ Tile.TILE_HEIGHT]];
	}// End getDoor method
	...
	private void loadTriggers(String path) {
		...
		ArrayList<String> roomNames = Utilities
				.loadFileArray("res/lists/rooms.txt");
		String name;
		...
		while (index < roomData.length) {
			...
			// Stores new Room object and coordinates in Room static array rooms
			// at this index
